...
class FlyingThings3D(BaseOpticalFlowDataset):
    def __init__(
        ...
        self.individual_quantity_folder = os.path.join(self.root, "individual_quantities", self.split)
        self.flow_folder = os.path.join(self.root, "flow", self.split)
        ...
        individual_quantity_index_search = self.individual_quantity_index.set_index(["split", "division", "seq", "idx"])
        ...
        self.image_pairs = []
        split_key = "TRAIN" if self.split == "train" else "TEST"
        for idx, row in self.flow_index.iterrows():
            img0_idx = individual_quantity_index_search.loc[(split_key, row["division"], row["seq"], row["pair0_id"])][
                "h5_reader_index"
            ]
            img1_idx = individual_quantity_index_search.loc[(split_key, row["division"], row["seq"], row["pair1_id"])][
                "h5_reader_index"
            ]
    ...
    def _get_raw_views(self, idx, rng: int):
        ...
        img1_dstype_sel = self.dstype if (not self.aug_clean_final or (selection < 0.5)) else self.other_dstype
        img2_dstype_sel = self.dstype if (not self.aug_clean_final or (selection < 0.5)) else self.other_dstype
            ...
            views.append(
                {
                    "img": img,
                    "flow": (
                        flow_data[f"{direction}_{view_name}_flow"].permute(2, 0, 1)
                        if flow_data[f"{direction}_{view_name}_flow"].shape[-1] == 2
                        else flow_data[f"{direction}_{view_name}_flow"]
                    ),
                    ...
                    "label": f"{metadata['division']}_{metadata['seq']}_{direction}_{img_dstype}",
...
if __name__ == "__main__":
    ...
    if args.profile:
        ...
        cProfile.run(
            "profile_dataset()",
            filename="/uniflowmatch/outputs/datasets_profile/flyingthings_profile_results",
        )

    if args.viz:
        rr.script_setup(args, f"FlyingThings_Dataloader")
