...
import h5py
import torch
from torch.multiprocessing import Manager, Process, Queue, current_process
...
class DistributedH5Reader:
    ...
    def read(self, index: int, keys: Optional[List[str]] = None) -> Dict[str, torch.Tensor]:
        """
        ...
        if index in self.index_to_file_map:
            ...
            with h5py.File(file_path, "r") as h5file:  # we *could* optimize for repeated file opening here
                read_keys = [k for k in h5file.keys() if k != "converted_to_rgb"] if keys is None else keys
                try:
                    return {key: torch.from_numpy(h5file[key][file_index]) for key in read_keys}
                except KeyError as e:
                    print(f"Error reading keys {read_keys} from file {file_path} at index {file_index}.")
                    raise e
        else:
    ...
    def _load_and_concatenate_index_maps(self) -> None:
        ...
        for file in os.listdir(self.base_dir):
            if file.endswith(".json"):
                ...
                if isinstance(loaded_map, dict) and not ("forward_match_errors" in loaded_map):
                    self.index_to_file_map.update({int(k): v for k, v in loaded_map.items()})
                else:
                    continue
