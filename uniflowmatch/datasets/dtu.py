#!/usr/bin/env python3
# --------------------------------------------------------
# Dataloader for preprocessed DTU dataset
...
class DTU(BaseStereoViewDataset):
    ...
    def _get_views(self, pair_idx, resolution, rng):
        ...
        for view_name in [img1_name, img2_name]:
            # Load the RGB image
            img_path = osp.join(scene_folder, "images", view_name + ".png")
            image = imread_cv2(img_path)
            # Load the depth data
            depth_path = osp.join(scene_folder, "depths", view_name + ".npy")
            depthmap = np.load(depth_path).astype(np.float32)
            # Load the camera parameters
            params_path = osp.join(scene_folder, "camera_params", view_name + "_intrinsics.npy")
            intrinsics = np.load(params_path).astype(np.float32)
            # Load the pose
            ...
            views.append(
                dict(
                    ...
                    dataset="DTU",
                    label=scene_name,
                    instance=scene_name + "_" + view_name,
...
def get_parser():
    ...
    parser.add_argument("-rd", "--root_dir", default="/mnt/xri_mapsresearch/data/nkeetha/dtu_processed", type=str)
    parser.add_argument(
        "-pmp",
        "--pair_metadata_path",
        default="/mnt/xri_mapsresearch/data/nkeetha/anymap_data_pairs/dtu_valid_pairs.npz",
        type=str,
    )
...
if __name__ == "__main__":
    ...
    if args.viz:
        rr.script_setup(args, f"DTU_Dataloader")
