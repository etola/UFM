...
class Spring(BaseOpticalFlowDataset):
    ...
    def _get_raw_views(self, idx, rng: int):
        ...
        file_parts = npz_filename.split("/")

        seq = file_parts[-4]
        direction = file_parts[-3]
        file_name = file_parts[-1]  # of the format "{04d}_{04d}.npz"

        idx_0 = int(file_name.split("_")[0])
        idx_1 = int(file_name.split("_")[1].split(".")[0])

        # read image file
        img0 = os.path.join(self.base_folder, seq, direction, "images", f"{idx_0:04d}.png")
        img1 = os.path.join(self.base_folder, seq, direction, "images", f"{idx_1:04d}.png")
        ...
        # read the npz file
        flow_data = np.load(npz_filename)

        views = []
        for i in range(2):
            views.append(
                {
                    "img": torch.from_numpy(img0 if i == 0 else img1),
                    "flow": torch.from_numpy(flow_data[f"flow_{i}"]),
                    "fov_mask": torch.from_numpy(flow_data[f"fov_mask_{i}"]),
                    "non_occluded_mask": torch.from_numpy(flow_data[f"non_occluded_mask_{i}"]),
                    "occlusion_supervision_mask": torch.from_numpy(flow_data[f"occlusion_supervision_mask_{i}"]),
                    "dataset": "spring",
                    "label": f"{self.split}_{seq}_{direction}",
                    "instance": f"{idx_0 if i == 0 else idx_1}",
...
def get_parser():
    ...
    parser.add_argument("-rd", "--root_dir", default="/uniflowmatch/data/spring_processed", type=str)
...
if __name__ == "__main__":
    ...
    if args.viz:
        rr.script_setup(args, f"Spring_Dataloader")
