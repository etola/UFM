"""
This file is for preprocessing the flyingthings dataset.
...
class ProcessFlyingThingsSingleQuantity(IndependentParallelProcessing):
    def __init__(self, root: str, output_path: str, split: str = "train", batch_size=128, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.root = root
        self.split = split
        self.batch_size = batch_size

        self.output_path = os.path.join(output_path, "individual_quantities", split)
    ...
    def get_total_job_args(self) -> List[Any]:
        ...
        if self.split == "train":
            image_dirs = sorted(glob(os.path.join(self.root, "frames_cleanpass", "TRAIN/*/*")))
        elif self.split == "test":
            image_dirs = sorted(glob(os.path.join(self.root, "frames_cleanpass", "TEST/*/*")))
        else:
        ...
        for image_dir in image_dirs:
            ...
            seq = int(path_parts[-1])
            division = path_parts[-2]
            split = path_parts[-3]

            for image_id in image_ids:
                job_args.append({"split": split, "division": division, "seq": seq, "idx": image_id})
            ...
            for i, (_, row) in tqdm(enumerate(job_arg.iterrows())):
                ...
                for direction in ["left", "right"]:
                    ...
                    pose_file = os.path.join(
                        self.root, "camera_data", row["split"], row["division"], f"{row['seq']:04d}", "camera_data.txt"
                    )
                    ...
                    try:
                        ...
                        print(
                            f"Missing pose for {row['split']} {row['division']} {row['seq']:04d} {direction} {row['idx']:04d}"
                        )
                ...
                final_save_data = {}
                final_save_data.update({k + "_left": v for k, v in data["left"].items()})
                final_save_data.update({k + "_right": v for k, v in data["right"].items()})
...
class ProcessFlyingThingsFlow(IndependentParallelProcessing):

    def __init__(self, root: str, processed_root: str, split: str = "train", batch_size=128, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.root = root
        ...
        self.output_path = os.path.join(processed_root, "flow", split)
        if not os.path.exists(self.output_path):
            os.makedirs(self.output_path)
    ...
    def get_total_job_args(self) -> List[Any]:
        ...
        job_batches = []
        for i in range(0, len(job_df), self.batch_size):
            job_batches.append(job_df.iloc[i : i + self.batch_size])

        return job_batches

    def on_before_process_processing(self, node_job_args, manager):
        # cache the reader to access our packed data
        self.indiv_quantity_reader = DistributedH5Reader(
            os.path.join(self.processed_root, "individual_quantities", self.split)
        )

        index_file = os.path.join(self.processed_root, "individual_quantities", self.split, "index.json")
        ...
        self.index_to_file_map.set_index(["split", "division", "seq", "idx"], inplace=True)
                ...
                for minibatch in tqdm(minibatches):
                    ...
                    for _, row in minibatch.iterrows():
                        ...
                        view0_individual_idx = int(
                            self.index_to_file_map.loc[
                                (row["split"], row["division"], row["seq"], row["pair0_id"]), "original_index"
                            ]
                        )
                        view1_individual_idx = int(
                            self.index_to_file_map.loc[
                                (row["split"], row["division"], row["seq"], row["pair1_id"]), "original_index"
                            ]
                        )
                        ...
                        flow_seq_folder = os.path.join(
                            self.root, "optical_flow", row["split"], row["division"], f"{row['seq']:04d}"
                        )
                        ...
                        disparity_seq_folder = os.path.join(
                            self.root, "disparity", row["split"], row["division"], f"{row['seq']:04d}"
                        )
                        ...
                        disparity_change_seq_folder = os.path.join(
                            self.root, "disparity_change", row["split"], row["division"], f"{row['seq']:04d}"
                        )
    ...
    def read_flyingthings(self, root: str, split: str, division: str, seq: int, idx: int, dstype="frames_cleanpass"):

        img_seq_folder = os.path.join(root, dstype, split, division, f"{seq:04d}")

        # img
        img_left = np.array(read_gen(os.path.join(img_seq_folder, f"left/{idx:04d}.png")))
        img_right = np.array(read_gen(os.path.join(img_seq_folder, f"right/{idx:04d}.png")))

        # disparity
        disparity_seq_folder = os.path.join(root, "disparity", split, division, f"{seq:04d}")
        disparity_left = read_gen(os.path.join(disparity_seq_folder, f"left/{idx:04d}.pfm"))
        disparity_right = read_gen(os.path.join(disparity_seq_folder, f"right/{idx:04d}.pfm"))

        # disparity change
        disparity_change_seq_folder = os.path.join(root, "disparity_change", split, division, f"{seq:04d}")
        disparity_change_future_left = read_gen(
            os.path.join(disparity_change_seq_folder, f"into_future/left/{idx:04d}.pfm")
        )
        disparity_change_future_right = read_gen(
            os.path.join(disparity_change_seq_folder, f"into_future/right/{idx:04d}.pfm")
        )

        disparity_change_past_left = read_gen(
            os.path.join(disparity_change_seq_folder, f"into_past/left/{idx:04d}.pfm")
        )
        disparity_change_past_right = read_gen(
            os.path.join(disparity_change_seq_folder, f"into_past/right/{idx:04d}.pfm")
        )

        # optical flow
        flow_seq_folder = os.path.join(root, "optical_flow", split, division, f"{seq:04d}")

        flow_future_left = read_gen(
            os.path.join(flow_seq_folder, f"into_future/left/OpticalFlowIntoFuture_{idx:04d}_L.pfm")
        )
        flow_future_right = read_gen(
            os.path.join(flow_seq_folder, f"into_future/right/OpticalFlowIntoFuture_{idx:04d}_R.pfm")
        )

        flow_past_left = read_gen(os.path.join(flow_seq_folder, f"into_past/left/OpticalFlowIntoPast_{idx:04d}_L.pfm"))
        flow_past_right = read_gen(
            os.path.join(flow_seq_folder, f"into_past/right/OpticalFlowIntoPast_{idx:04d}_R.pfm")
        )

        # camera pose
        pose_file = os.path.join(root, "camera_data", split, division, f"{seq:04d}", "camera_data.txt")
        pose = parse_pose_file(pose_file)

        pose_left = pose[idx]["left"]
        pose_right = pose[idx]["right"]
        ...
        # compute derived data - depth
        depth_left = fx * 1.0 / disparity_left
        depth_right = fx * 1.0 / disparity_right
        ...
        return {
            ...
            "disparity_change_future_left": disparity_change_future_left,
            "disparity_change_future_right": disparity_change_future_right,
            "disparity_change_past_left": disparity_change_past_left,
            "disparity_change_past_right": disparity_change_past_right,
            "flow_future_left": flow_future_left,
            "flow_future_right": flow_future_right,
            "flow_past_left": flow_past_left,
            "flow_past_right": flow_past_right,
            "pose_left": pose_left,
...
if __name__ == "__main__":
    parser = ProcessFlyingThingsFlow.get_default_parser()

    parser.add_argument("--root", type=str, default="/uniflowmatch/data/raw_data/FlyingThings3D")
    parser.add_argument(
        "--output_path", type=str, default="/uniflowmatch/data/flyingthings3d_preprocessed"
    )
    ...
    process = ProcessFlyingThingsSingleQuantity(**vars(args), split="train")
    process.run()

    process = ProcessFlyingThingsSingleQuantity(**vars(args), split="test")
    process.run()

    process = ProcessFlyingThingsFlow(**vars(args), split="train", processed_root=args.output_path)
    process.run()

    process = ProcessFlyingThingsFlow(**vars(args), split="test", processed_root=args.output_path)
    process.run()
