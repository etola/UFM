#!/usr/bin/env python3
# --------------------------------------------------------
# Code adopted from https://github.com/naver/dust3r/blob/main/datasets_preprocess/preprocess_blendedMVS.py
# --------------------------------------------------------
...
# 5) python preprocess_datasets/preprocess_blendedMVS.py --blendedmvs_dir /path/to/tmp/blendedMVS/ --output_dir /path/to/blendedmvs_processed
# --------------------------------------------------------
import os
...
def get_parser():
    ...
    parser.add_argument("--blendedmvs_dir", default="/path/to/blendedmvs")
    parser.add_argument("--output_dir", default="/path/to/data_root/blendedmvs_processed")
    return parser
...
def load_crop_and_save(root, img, out_dir):
    ...
    color_image_in = cv2.cvtColor(
        cv2.imread(osp.join(root, "blended_images", img + ".jpg"), cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB
    )
    ...
    assert H * 4 == W * 3
    image, depthmap, intrinsics_out, R_in2out = _crop_image(intrinsics_in, color_image_in, depthmap_in, (746, 560))
